define(["moment", "numeral", "tablesort", "tablesort.numeric"],
  ...
  function showLoadBar(className, v, nproc) {
    var span = document.createElement("span")
    span.classList.add("bar")
    span.classList.add(className)
    ...
    bar.style.width = (v / nproc * 100) + "%"
    bar.style.background = "rgba(255, 125, 50, 0.9)"
    span.style.background = "rgba(255, 125, 50, 0.6)"
    span.appendChild(bar)
    }
    else
    {
  ...
  function showAirtimeBar(className, v) {
    ...
    var label = document.createElement("label")
    label.textContent = Math.round((1 - v.free) * 100) + " %"
    span.appendChild(label)
  ...
  function showLoad(d) {
    ...
    return function (el) {
      el.appendChild(showLoadBar("load-avg", d.statistics.loadavg, d.nodeinfo.hardware.nproc))
    }
  }
  ...
  function showGateway(d, router) {
    ...
    return function (el) {
      ...
      while (gw && nh && gw.id !== nh.id && num < 10) {
        ...
        if (dictGet(nh.node.statistics, ["gateway"]) == null) break
