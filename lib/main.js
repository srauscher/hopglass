...
function (moment, Router, L, GUI, numeral) {
  return function (config) {
    ...
    var lat = localStorage.getItem(pathname + "Latitude")
    var lng = localStorage.getItem(pathname + "Longitude")

    if ((typeof lat === "string" && !isNaN(lat)) && (typeof lng === "string" && !isNaN(lng)))
      config.fixedCenter = {"lat": lat, "lng": lng, "radius": 5}
