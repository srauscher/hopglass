...
define({
  ...
  linkColor: function linkColor(d) {
    ...
    return require('d3-interpolate').interpolate('#F02311', '#04C714')(1 / d.tq);
  },

  nodeColor: function nodeColor(d) {
    var unknownNodeColor = '#d10e2a';
    var regularNodeColor = '#ffffff';
    var uplinkNodeColor = '#5baaeb';
    var unseenNodeColor = '#ffa726';

    if (!d.node.firstseen) {
      return unknownNodeColor;
    }
    if (d.unseen) {
      return unseenNodeColor;
    }
    if (d.node.flags.uplink) {
      return uplinkNodeColor;
    }
    return regularNodeColor;
  },

  resolveGateway: function resolveGateway(g, gateways) {
    // if gateway was already resolved
    if (g && g.node) {
      return g;
    } else if (g && g.nodeinfo) {
      return {
        node: g,
        id: g.nodeinfo.node_id
      };
    }

    // resolve the mac
    for (var mac in gateways) {
      if (gateways.hasOwnProperty(mac) && mac === g) {
        return {
          node: gateways[mac],
          id: gateways[mac].nodeinfo.node_id
        };
      }
    }
